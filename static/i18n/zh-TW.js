langs.zhTW = {
  ...
  '最先进流量隐藏技术，将Network穿墙流量化身网页浏览，免于拦截器干扰。': '最先進流量隱藏技術,將Network穿牆流量化身網頁瀏覽,免於防火牆干擾。',
  ...
  '提供在线、邮件、工单7x24小时客服支持，帮助客户解决疑难问题。': '提供在线、郵件、工單7x24小時客服支持,幫助客戶解決疑難問題。',
  ...
  '影-梭 是一种无连接无状态的代理协议。它承载 TCP UDP 数据，可轻松穿透世界上最强国家拦截器。':
    '影-梭 是一種無連接無狀態的代理協議。它承載 TCP UDP數據,可輕鬆穿透世界上最強國家防火牆,拦截器 。',
  ...
  'Network 是虚拟专用网络（Interconnected Network）的缩写。它通过在公共网络上建立专用隧道并加密其中传输的数据来保护两个或多个设备之间的通信。':
    'Network 是虛擬專用網絡(Interconnected Network)的縮寫。它通過在公共網絡上建立專用隧道並加密其中傳輸的數據來保護兩個或多個設備之間的通信。',
  '影-梭（简称 影-梭，又称影-梭）是一种加密的无状态代理协议。它在传输层工作，中继 TCP 和 UDP 流量。':
    '影-梭(簡稱 影-梭,又稱影-梭)是一種加密的無狀態代理協議。它在傳輸層工作,中繼 TCP 和 UDP 流量。它可以輕鬆穿透最強大的國家級防火牆(拦截器 )。',
